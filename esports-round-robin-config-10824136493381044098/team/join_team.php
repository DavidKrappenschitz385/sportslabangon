        ...
        if (!isset($error)) {
            ...
            if ($check_stmt->rowCount() > 0) {
                ...
                if ($member_stmt->rowCount() > 0) {
                    ...
                    $league_id = $team['league_id'] ?? null; // Can be null now
                    ...
                    if ($league_id === null) {
                         $insert_stmt->bindValue(':league_id', null, PDO::PARAM_NULL);
                    } else {
...
$where_conditions = ["(l.status IN ('open', 'active') OR l.id IS NULL)"]; // Allow teams without leagues or in active leagues
$params = [];
...
// Get available teams
$teams_query = "SELECT t.*, t.registration_deadline as team_deadline, l.name as league_name, l.season, l.status as league_status,
                       l.registration_deadline as league_deadline, l.max_teams,
...
$pending_requests_query = "SELECT rr.*, t.name as team_name, l.name as league_name, l.season, s.name as sport_name
                           ...
                           LEFT JOIN leagues l ON rr.league_id = l.id
                           LEFT JOIN sports s ON l.sport_id = s.id
...
<body>
    ...
    <div class="container">
        ...
        <?php if (count($user_pending_requests) > 0 || true): // Always show section for history access ?>
            <div class="pending-requests-section">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    <h3 style="color: #856404;">Your Requests</h3>
                    <div class="tabs">
                        <button onclick="toggleRequests('pending')" class="btn btn-sm btn-primary">Pending</button>
                        <button onclick="toggleRequests('history')" class="btn btn-sm btn-secondary">History</button>
                    </div>
                </div>
                ...
                <div id="history-requests" style="display: none;">
                    <?php
                    // Get request history
                    $history_query = "SELECT rr.*, t.name as team_name, l.name as league_name
                                      FROM registration_requests rr
                                      JOIN teams t ON rr.team_id = t.id
                                      LEFT JOIN leagues l ON rr.league_id = l.id
                                      WHERE rr.player_id = :user_id AND rr.status != 'pending'
                                      ORDER BY rr.processed_at DESC LIMIT 10";
                    $history_stmt = $db->prepare($history_query);
                    $history_stmt->bindParam(':user_id', $current_user['id']);
                        ...
                        <div class="request-card" style="opacity: 0.8;">
                            <div class="request-header">
                                <div class="request-team">
                                    ...
                                    <span style="padding: 2px 5px; border-radius: 3px; color: white; background: <?php echo $req['status'] == 'approved' ? '#28a745' : '#dc3545'; ?>; font-size: 0.8em; margin-left: 5px;">
                                        <?php echo ucfirst($req['status']); ?>
                                    </span>
                                </div>
                                <small><?php echo date('M j, Y', strtotime($req['processed_at'])); ?></small>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    <?php else: ?>
            ...
            <script>
                function toggleRequests(type) {
                    if (type === 'pending') {
                        ...
                        document.getElementById('history-requests').style.display = 'block';
                    }
                }
            </script>
        <?php endif; ?>
        ...
        <div class="teams-section">
            ...
            <div class="teams-grid">
                ...
                <div class="team-card">
                    ...
                    // If team has a deadline, use it. Otherwise use league deadline.
                    $effective_deadline = $team['team_deadline'] ?? $team['league_deadline'];

                    // If deadline is null, treat as open (false)
                    ...
                    <div class="team-header">
                        <div>
                            ...
                            <div class="team-league">
                                ...
                                <?php else: ?>
                                    <span style="color: #666; font-style: italic;">Not in a league</span>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <div class="team-stats">
                        ...
                        <div class="team-stat">
                            <div class="team-stat-number">
                                <?php echo $effective_deadline ? date('M j', strtotime($effective_deadline)) : 'Open'; ?>
                            </div>
                    ...
                    <div style="display: flex; gap: 10px;">
                        ...
                        <?php else: ?>
                            <button class="join-btn" style="flex: 1;" onclick="openJoinModal(
                                <?php echo $team['id']; ?>,
                        ...
                        <a href="messages.php?recipient_id=<?php echo $team['owner_id']; ?>&team_id=<?php echo $team['id']; ?>" class="btn btn-primary" style="display: flex; align-items: center; justify-content: center;">
                            Message
                        </a>
                    </div>
                </div>
