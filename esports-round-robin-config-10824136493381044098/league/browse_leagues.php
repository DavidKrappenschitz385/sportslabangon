...
$check_owner_query = "SELECT 1 FROM teams WHERE owner_id = :user_id";
$check_owner_stmt = $db->prepare($check_owner_query);
$check_owner_stmt->bindParam(':user_id', $current_user['id']);
$check_owner_stmt->execute();
$is_team_owner = $check_owner_stmt->rowCount() > 0;

// Check if user is a team member (active)
$check_member_query = "SELECT 1 FROM team_members WHERE player_id = :user_id AND status = 'active'";
$check_member_stmt = $db->prepare($check_member_query);
$check_member_stmt->bindParam(':user_id', $current_user['id']);
            ...
            <div class="league-card">
                ...
                <div class="league-body">
                    ...
                    <div class="league-actions">
                        ...
                        <!-- Join Button -->
                        <?php
                        if (in_array($league['id'], $user_league_participations)) {
                            // Already joined this league
                            echo '<span class="btn btn-secondary disabled">Joined</span>';
                        } else if ($league['status'] == 'open') {
                            // Determine if Join button is enabled
                            $join_disabled = ($is_team_member && !$is_team_owner);

                            if ($join_disabled) {
                                // Member but not owner -> Disabled
                                echo '<button class="btn btn-secondary disabled" disabled>Join</button>';
                            } else {
                                // Owner OR Not in a team -> Enabled
                                echo '<a href="../team/register_team.php?league_id=' . $league['id'] . '" class="btn btn-primary">Join</a>';
                            }
                        }
                        ?>
                        ...
                        if (!$has_messaged) {
                            $msg_link .= "&message=Inquiry about League: " . urlencode($league['name']);
                        }
                        ?>
                        <a href="<?php echo $msg_link; ?>" class="btn btn-warning"><?php echo $msg_text; ?></a>
