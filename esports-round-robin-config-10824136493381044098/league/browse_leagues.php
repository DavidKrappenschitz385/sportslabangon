...
// Check if user is in ANY team (as owner or member)
$any_team_query = "SELECT 1 FROM teams WHERE owner_id = :user_id
                   UNION
                   SELECT 1 FROM team_members WHERE player_id = :user_id AND status = 'active'
                   LIMIT 1";
$any_team_stmt = $db->prepare($any_team_query);
$any_team_stmt->bindParam(':user_id', $current_user['id']);
$any_team_stmt->execute();
$is_in_any_team = $any_team_stmt->rowCount() > 0;
            ...
            <div class="featured-grid">
                ...
                <div class="featured-card">
                    ...
                    <div style="margin-top: 1.5rem;">
                        <a href="view_league.php?id=<?php echo $featured['id']; ?>" class="btn btn-info">Details</a>

                        <?php
                            $is_messaged = in_array($featured['created_by'], $messaged_users);
                            $msg_text = $is_messaged ? "Messaged" : "Message";
                            $msg_class = $is_messaged ? "btn-secondary" : "btn-warning";
                        ?>
                        <a href="../team/messages.php?recipient_id=<?php echo $featured['created_by']; ?>&message=Inquiry about League: <?php echo urlencode($featured['name']); ?>" class="btn <?php echo $msg_class; ?>">
                            <?php echo $msg_text; ?>
                        </a>

                        <?php if ($is_in_any_team): ?>
                             <span class="btn btn-secondary disabled">Join (Disabled)</span>
                        <?php else: ?>
            ...
            <div class="league-card">
                ...
                <div class="league-body">
                    ...
                    <div class="league-actions">
                        ...
                        <?php
                            $is_messaged = in_array($league['created_by'], $messaged_users);
                            $msg_text = $is_messaged ? "Messaged" : "Message";
                            $msg_class = $is_messaged ? "btn-secondary" : "btn-warning";
                        ?>
                        <a href="../team/messages.php?recipient_id=<?php echo $league['created_by']; ?>&message=Inquiry about League: <?php echo urlencode($league['name']); ?>" class="btn <?php echo $msg_class; ?>">
                            <?php echo $msg_text; ?>
                        </a>

                        <?php if ($is_in_any_team): ?>
                            <!-- Quick create team hidden for users already in a team -->
                            <?php if (in_array($league['id'], $user_league_participations)): ?>
                                <span class="btn btn-secondary disabled">Joined</span>
                            <?php else: ?>
                                <span class="btn btn-secondary disabled">Join</span>
                            <?php endif; ?>
                        <?php else: ?>
